...
- **Injector authoring contract (resolved by design-v3, fully implemented).** Injection is a typed, scheme-aware request transform: an injector selects a `scheme` (+ `params`) the proxy implements, replacing v2's `format`/substring-replace. The `format` field is gone. All of design-v3 now ships: substitute + sign (`sigv4`, plus `ovh`/`jwt-bearer` as scripts) + re-seal (`oauth2-reseal` + `mint` primitives, TTL'd runtime layer), and the Starlark escape hatch on the flat/implicit-ctx "option B" primitive API (`api`-versioned manifests). See `design-v3.md` / `docs/injectors.md`; don't reintroduce `format` or a single-mechanism substitution, and don't revert the scripted API to explicit ctx-threading.
