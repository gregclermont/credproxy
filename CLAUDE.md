...
**Bindings**: a *binding* ties an injector (how a credential is shaped into a request — which typed *scheme* the proxy runs) to a provider (where the value comes from), scoped to one or more hosts. The host CLI resolves each binding's secret(s) from its provider at `start`/`apply`/`binding test` time and pushes the resolved wire config (`{"bindings": [{name, hosts, scheme, params, secret:{slot:value}, placeholder, env}]}`) to `/admin/config`; the proxy dispatches on `scheme` (substitute family `bearer`/`basic`/`body` implemented; sign family later). The provider exec protocol is batch-native (`{op:"get", secrets:[refs]}` → `{values:{ref:val}}`), one invocation per binding. The real secret never enters the workspace. The inward `/setup` endpoint exposes only least-disclosure fields (name, placeholder, env, scheme, params, hosts — never secret values/provider/secret-id) so the workspace can self-configure. The scheme model is specified in `design-v3.md`.
...
- **Injector authoring contract (resolved by design-v3).** Injection is now a typed, scheme-aware request transform: an injector selects a `scheme` (+ `params`) the proxy implements, replacing v2's `format`/substring-replace. The `format` field is gone. Still open within design-v3: the sign family (`sigv4`/`hmac`/`jwt-bearer`), the Starlark scripted-injector escape hatch, and re-seal — built on seams already in place (two-phase `on_request`/`on_response`, runtime-augmentable substitution layer). See `design-v3.md`; don't reintroduce `format` or a single-mechanism substitution.
